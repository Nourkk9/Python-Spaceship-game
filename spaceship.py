...
class Menu():
    def __init__(self):
        pygame.init()
        size:list = [SCREEN_WIDTH, SCREEN_HEIGHT]
    ...
    def load_players_menu(self):
        while not self.done:
            ...
            if self.current_player < 0 :
                ...
                text = font.render("Click on the player name to start the game", True, WHITE)
                center_x = (SCREEN_WIDTH // 2) - (text.get_width() // 2)
                center_y = (SCREEN_HEIGHT // 2)-150 - (text.get_height() // 2)
